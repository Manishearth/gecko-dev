                                     ...
                                     GenericSpecifiedValues* aData)
{
    ...
    if (value && value->Type() == nsAttrValue::eEnum) {
      ...
      case NS_STYLE_TEXT_ALIGN_LEFT:
        if (!aData->PropertyIsSet(eCSSProperty_margin_left))
          aData->SetPixelValue(eCSSProperty_margin_left, 0.0f);
      ...
      case NS_STYLE_TEXT_ALIGN_RIGHT:
        ...
        if (!aData->PropertyIsSet(eCSSProperty_margin_right))
          aData->SetPixelValue(eCSSProperty_margin_right, 0.0f);
        break;
  ...
  if ((aData->CanHoldStyleStruct(NS_STYLE_INHERIT_BIT(Border))) && noshade) {
    // size: integer
    ...
    if (!aData->PropertyIsSet(eCSSProperty_border_top_width))
      aData->SetPercentValue(eCSSProperty_border_top_width, sizePerSide);
    if (allSides) {
      if (!aData->PropertyIsSet(eCSSProperty_border_right_width))
        aData->SetPercentValue(eCSSProperty_border_right_width, sizePerSide);
      if (!aData->PropertyIsSet(eCSSProperty_border_bottom_width))
        aData->SetPercentValue(eCSSProperty_border_bottom_width, sizePerSide);
      if (!aData->PropertyIsSet(eCSSProperty_border_left_width))
        aData->SetPercentValue(eCSSProperty_border_left_width, sizePerSide);
    }
    ...
    if (allSides) {
      ...
      for (const nsCSSPropertyID* props =
        ...
        if (!aData->PropertyIsSet(*props))
          aData->SetPixelValue(*props, 10000.0f);
        }
    }
  }
  if (aData->CanHoldStyleStruct(NS_STYLE_INHERIT_BIT(Color))) {
    // color: a color
    // (we got the color attribute earlier)
    if (colorIsSet &&
        !aData->PropertyIsSet(eCSSProperty_color) &&
