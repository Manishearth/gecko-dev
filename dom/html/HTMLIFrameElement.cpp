    ...
    if (value && value->Type() == nsAttrValue::eEnum) {
      ...
      if (NS_STYLE_FRAME_0 == frameborder ||
        ...
        if (!aData->PropertyIsSet(eCSSProperty_border_right_width))
          aData->SetPixelValue(eCSSProperty_border_right_width, 0.0f);
        if (!aData->PropertyIsSet(eCSSProperty_border_bottom_width))
          aData->SetPixelValue(eCSSProperty_border_bottom_width, 0.0f);
        if (!aData->PropertyIsSet(eCSSProperty_border_left_width))
          aData->SetPixelValue(eCSSProperty_border_left_width, 0.0f);
      }
    }
  }
