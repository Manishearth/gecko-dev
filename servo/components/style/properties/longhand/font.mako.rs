...
<%helpers:longhand name="font-size" need_clone="True" animation_value_type="ComputedValue"
    ...
    pub mod computed_value {
        ...
        pub struct KeywordInfo {
            ...
            pub factor: f32,
            pub offset: NonNegativeAu,
        }

        impl KeywordInfo {
            /// Given a parent keyword info (self), apply an additional factor/offset to it
            pub fn compose(self, factor: f32, offset: NonNegativeAu) -> Self {
                KeywordInfo {
                    kw: self.kw,
    ...
    % if product == "servo":
        impl ToComputedValue for KeywordSize {
            ...
            #[inline]
            fn to_computed_value(&self, _: &Context) -> NonNegativeAu {
            ...
            #[inline]
            fn from_computed_value(_: &NonNegativeAu) -> Self {
                unreachable!()
            }
        }
    % else:
        impl ToComputedValue for KeywordSize {
            type ComputedValue = NonNegativeAu;
            #[inline]
            fn to_computed_value(&self, cx: &Context) -> NonNegativeAu {
    ...
    pub fn get_initial_value() -> computed_value::T {
        computed_value::T {
            size: NonNegativeAu::from_px(FONT_MEDIUM_PX),
            info: Some(computed_value::KeywordInfo {
               ...
               context.builder.get_parent_font().gecko().mGenericID {
                if let Some((kw, ratio, offset)) = context.builder.font_size_keyword {
                    computed = computed_value::T {
                        size: context.maybe_zoom_text(kw.to_computed_value(context).scale_by(ratio) + offset),
                        info: Some(computed_value::KeywordInfo {
                            kw: kw,
                            factor: ratio,
                            offset: offset
